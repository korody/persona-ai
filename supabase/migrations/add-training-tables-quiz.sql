...
CREATE POLICY IF NOT EXISTS "Public read active examples" ON avatar_conversation_examples
  FOR SELECT USING (is_active = true);

CREATE POLICY IF NOT EXISTS "Service role all examples" ON avatar_conversation_examples
  FOR ALL USING (auth.jwt()->>'role' = 'service_role');

CREATE POLICY IF NOT EXISTS "Public read chunks" ON knowledge_chunks
  FOR SELECT USING (true);

CREATE POLICY IF NOT EXISTS "Service role all chunks" ON knowledge_chunks
  FOR ALL USING (auth.jwt()->>'role' = 'service_role');

CREATE POLICY IF NOT EXISTS "Users view own memory" ON user_memory
  FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Service role all memory" ON user_memory
  FOR ALL USING (auth.jwt()->>'role' = 'service_role');

CREATE POLICY IF NOT EXISTS "Users view own prefs" ON user_communication_preferences
  FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users update own prefs" ON user_communication_preferences
  FOR UPDATE USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Service role all prefs" ON user_communication_preferences
  FOR ALL USING (auth.jwt()->>'role' = 'service_role');

CREATE POLICY IF NOT EXISTS "Users create feedback" ON conversation_feedback
  FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users view own feedback" ON conversation_feedback
  FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Service role all feedback" ON conversation_feedback
  FOR ALL USING (auth.jwt()->>'role' = 'service_role');

CREATE POLICY IF NOT EXISTS "Service role all patterns" ON learned_patterns
  FOR ALL USING (auth.jwt()->>'role' = 'service_role');

CREATE POLICY IF NOT EXISTS "Service role all highlighted" ON highlighted_conversations
  FOR ALL USING (auth.jwt()->>'role' = 'service_role');
