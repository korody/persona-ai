...
DROP FUNCTION IF EXISTS get_active_campaign(VARCHAR);
...
-- 5. Função para buscar produtos recomendados (DROP antes de recriar)
...
DROP TRIGGER IF EXISTS campaigns_updated_at ON avatar_campaigns;
CREATE TRIGGER campaigns_updated_at
  BEFORE UPDATE ON avatar_campaigns
  FOR EACH ROW
  EXECUTE FUNCTION update_marketing_updated_at();

DROP TRIGGER IF EXISTS products_updated_at ON avatar_products;
